package com.ktmmoe.shared.data.models.impls

import com.google.firebase.auth.FirebaseUser
import com.ktmmoe.shared.data.models.AuthenticationModel
import com.ktmmoe.shared.network.auth.AuthManager
import com.ktmmoe.shared.network.auth.FirebaseAuthManager

/**
 * Created by ktmmoe on 08, December, 2020
 **/
object AuthenticationModelImpl : AuthenticationModel {
    override var mAuthManager: AuthManager = FirebaseAuthManager

    override fun login(
            email: String,
            password: String,
            onSuccess: () -> Unit,
            onFailure: (String) -> Unit
    ) {
        mAuthManager.login(email, password, onSuccess, onFailure)
    }

    override fun register(
            email: String,
            password: String,
            userName: String,
            onSuccess: () -> Unit,
            onFailure: (String) -> Unit
    ) {
        mAuthManager.register(email, password, userName, onSuccess, onFailure)
    }
    override fun getFirebaseUser(onSuccess: (FirebaseUser?) -> Unit) {
        onSuccess(mAuthManager.getFirebaseUser())
    }

    override fun logout(onSuccess: () -> Unit) {
        onSuccess.invoke()
    }
}